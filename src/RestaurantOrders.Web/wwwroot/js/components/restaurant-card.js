import { escapeHtml, priceCategoryLabel } from "../utils/format.js";
...
export function restaurantCard(restaurant) {
  ...
  const price = priceCategoryLabel(restaurant.priceCategory);
  ...
  return `<a href="/restaurant.html?slug=${encodeURIComponent(restaurant.slug)}" class="restaurant-card group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200">
    ...
    <div class="p-6">
      ...
      <div class="mb-3 flex items-center justify-between"><span class="text-xs font-semibold uppercase tracking-widest text-amber-700">${restaurant.cuisines.join(" · ")}</span><span class="text-sm">★ ${rating}</span></div>
      ...
      <p class="mt-5 text-sm font-medium text-stone-800">${restaurant.city || ""} · ${price}</p>
